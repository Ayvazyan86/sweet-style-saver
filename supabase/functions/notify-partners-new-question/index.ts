  ...
  try {
    ...
    if (templateData?.template) {
      const variables: Record<string, string> = {
        ...
        details_line: record.details ? `📝 <b>Детали:</b>\n${record.details}` : '',
      }
      questionMessage = replaceVariables(templateData.template, variables)
    } else {
      questionMessage = `
❓ <b>Новый вопрос!</b>
...
${record.details ? `\n📝 <b>Детали:</b>\n${record.details}` : ''}
    ...
    for (const partner of sortedPartners) {
      ...
      if (result.success) {
        ...
        // Записываем в question_publications
        await supabase.from('question_publications').insert({
    ...
    if (sentCount > 0) {
      ...
      if (userProfile?.telegram_id) {
        ...
        const userMessage = `
✅ <b>Ваш вопрос отправлен экспертам!</b>

${partnerLinks || 'Ссылки на экспертов будут доступны позже.'}
